<template>
  <div class="wiki">
    ...
    <p>
      Если вы не хотите использовать OAuth, то методы
      <codes>getUserSessionByOAuthAccessToken</codes>,
      <codes>refreshAccessToken</codes> должны возвращать <b>null</b>, а метод
      <codes>createOAuthSession</codes> должен возвращать только
    ...
    <ul>
      ...
      <li>
        ...
        null. Если вы не передаете accessToken/refreshToken, то вы не
        используете OAuth
      </li>
    ...
    <ul>
      ...
      <li>
        getUserByLoginUrl - [опционально] при отсутствии используется
        getUserByUsernameUrl
      </li>
      <li>getUserByUUIDUrl - [обязателен]</li>
      <li>getUserSessionByOAuthAccessTokenUrl - [обязателен для OAuth]</li>
      <li>
        getAuthDetailsUrl - [опционально] при использовании 2FA/MFA и других
        способов кроме пароля
      </li>
      <li>
        refreshAccessTokenUrl - [опционально] при использовании OAuth с
        лимитированым accessToken
      </li>
      <li>
        verifyPasswordUrl - [опционально] при отсутствии passwordVerifer
        является обязательным
      </li>
      <li>createOAuthSessionUrl - [обязателен]</li>
      <li>
        updateServerIdUrl - [обязателен при отсутствии joinServerUrl и
        checkServerUrl]
      </li>
    ...
    <p>
      <b>Проверка что запрос пришел от лаунчсервера</b>. Лаунчсервер использует
      Bearer авторизацию для отправки запросов. Токен указывается в поле
      <codes>bearerToken</codes>. Вы должны проверить его, преджде чем
      обрабатывать запрос.
    </p>
    <p>
      Если вам нужно вернуть <b>null</b> вы должны отдать пустоту в теле ответа
    </p>
    <p>
      При нормальной работе скрипта он должен возвращать только 2XX коды. Любой
      другой код будет считатся ошибкой на стороне скрипта
    </p>
    <p>
      <b>Блок пользователя</b>(класс <codes>JsonUser</codes>). В дальнейшем
      будет обозначатся как <codes>[[JsonUser]]</codes>
    </p>
    <pcode
      autodetect
      code='
{
  ...
  "password": "PASSWORD HASH"
    ...
    />
    <p>
      Поле <codes>password</codes> является необходимым только если вы хотите
      использовать passwordVerifer вместо отправки запроса на verifyPasswordUrl
    </p>
    <p>
      Поле <codes>accessToken</codes>(minecraftAccessToken) является
      обязательным для запроса getUserSessionByOAuthAccessTokenUrl
    </p>
    <p>
      <b>Блок сессии</b>(класс <codes>JsonUserSession</codes>). В дальнейшем
      будет обозначатся как <codes>[[JsonUserSession]]</codes>
    </p>
    <pcode
      autodetect
      code='
{
  ...
  "user": [[JsonUser]]
    ...
    />
    <p>
      <b>Блок AuthReport</b>(класс <codes>JsonAuthReportResponse</codes>). В
      дальнейшем будет обозначатся как <codes>[[JsonAuthReportResponse]]</codes>
    </p>
    <pcode
      autodetect
      code='
          ...
          code='
{
  "uuid": "UUID"
    ...
    '
      ...
      <li>
        <b>getUserSessionByOAuthAccessTokenUrl</b>
        <p>Запрос:</p>
        <pcode
          autodetect
          code='
